...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="uv", description="Run the pypelines ASGI app (wrapper around uvicorn)"
    )
    ...
    test.add_argument(
        "--junitxml", help="Write junit-xml to the given path (passed to pytest)"
    )
    ...
    typecheck.add_argument(
        "--strict", action="store_true", help="Run mypy in strict mode (if configured)"
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    if args.cmd == "run":
        ...
        uvicorn.run(
            "pypelines.main:app", host=args.host, port=args.port, reload=args.reload
        )
        return 0
    ...
    if args.cmd == "lint":
        ...
        if not ruff:
            print(
                "ruff not found in PATH; ensure dev extras are installed",
                file=sys.stderr,
            )
            return 2
        cmd = [ruff, "check", "."]
    ...
    if args.cmd == "typecheck":
        ...
        if not mypy:
            print(
                "mypy not found in PATH; ensure dev extras are installed",
                file=sys.stderr,
            )
            return 2
        cmd = [mypy, "src", "tests"]
