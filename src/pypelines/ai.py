...
class ChromaVectorDB:
    ...
    def __init__(
        ...
        if not LANGCHAIN_AVAILABLE:
            raise RuntimeError(
                "Required langchain/chromadb packages not installed; install the 'ai' extra."
            )
    ...
    def _init_store(self) -> None:
        ...
        if self.persist_directory:
            ...
            self.store = Chroma(
                embedding_function=self.embeddings, collection_name=self.collection_name
            )
    ...
    def similarity_search(self, query: str, k: int = 4) -> List[Document]:
        hits: List[Document] = cast(
            List[Document], self.store.similarity_search(query, k=k)
        )
        return hits
...
class OllamaWrapper:
    def __init__(self, model: str = DEFAULT_MODEL, server_url: Optional[str] = None):
        ...
        self.model = model
        self.client = (
            Ollama(model=model, base_url=server_url)
            if server_url
            else Ollama(model=model)
        )
